				...
				if ( response && response.data && response.data.status && ( 'created' == response.data.status || 'updated' == response.data.status ) ) {
					...
					if ( $('.group-raw[data-group-id="'+groupID+'"]').length > 0 ) {
						...
						if ( parentID > 0 ) {
							...
							if ( $('.brm-menu').children('ul.brm-groups-admin').length > 0 )
								$('.brm-menu').children('ul.brm-groups-admin').append( '<li class="brm-group-li" data-group="'+groupId+'">' + groupRaw + '</li>' );
							else
								$('.brm-menu').append( '<ul class="brm-groups-admin"><li class="brm-group-li" data-group="'+groupId+'">' + groupRaw + '</li></ul>' );
						}
					}
					orderNestingGroupsItems();
				}
			...
			success: function(response){
				...
				if ( response && response.data && response.data.status && ( 'created' === response.data.status || 'updated' === response.data.status ) ) {
					...
					if ( $('.item-raw[data-item-id="'+itemId+'"]').length > 0 ) {
						...
						if ( $('.group-raw[data-group-id="'+groupId+'"]').closest('li').children('ul.brm-items-admin').length > 0 )
							$('.group-raw[data-group-id="'+groupId+'"]').closest('li').children('ul.brm-items-admin').append( '<li class="mjs-nestedSortable-no-nesting">' + itemRaw + '</li>' );
						else
							$('.group-raw[data-group-id="'+groupId+'"]').after( '<ul class="brm-items-admin"><li class="mjs-nestedSortable-no-nesting">' + itemRaw + '</li></ul>' );
					}
