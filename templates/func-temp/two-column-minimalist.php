<?php
/**
 * Two Column Minimalist Template Functions.
 *
 * @package Best_Restaurant_Menu/Templates/Functions/Two_Column_Menamalist
 * @author  PriceListo
 */
 ...
 * @return mixed|HTML
 */
function brm_column_minimalist_render_group_heading( $group, $is_subgroup = false ) {
	ob_start();
	?>
 ...
 * @return mixed|HTML
 */
function brm_column_minimalist_render_items( $items, $currency, $is_subgroup = false ) {
	ob_start();
	?>
 ...
 * @return mixed|HTML
 */
function brm_column_minimalist_renders_frontend_menu( $menu, $currency, $is_child = false ) {
	$html        = '';
	$child_class = $is_child ? ' nested-child' : '';
	foreach ( $menu as $group ) :
		$html .= '<div class="brm-menu-section' . $child_class . '">';
		$html .= brm_column_minimalist_render_group_heading( $group, $is_child );
		if ( isset( $group->items ) && ! empty( $group->items ) ) :
			$html .= brm_column_minimalist_render_items( $group->items, $currency, $is_child );
		endif;

		if ( isset( $group->childs ) && ! empty( $group->childs ) ) :
			$html .= brm_column_minimalist_renders_frontend_menu( $group->childs, $currency, true );
		endif;
		$html .= '</div>';
	endforeach;
